...
(function () {
  ...
  function sortAll (opts) {
    _groups.map(function (group) {
      ...
      const exposedValue = optsGroup[exposedVariable] || null;
      const reverse = (typeof optsGroup.reverse !== 'undefined') ? optsGroup.reverse : false;

      if (exposedVariable) {
        if (exposedValue) {
          // exposedValue could be something like [ 'tag1', 'tag2', 'tag3' ]
          for (let prop in exposedValue) {
            group[prop].files = reverse
              ? group[prop].files.sort(utils.orderByDateReverse)
              : group[prop].files.sort(utils.orderByDate);
          }
        } else {
          group[exposedVariable].files = reverse
            ? group[exposedVariable].files.sort(utils.orderByDateReverse)
            : group[exposedVariable].files.sort(utils.orderByDate);
        }
      } else {
